...
public class EmpDeptSalgradeTests
{
    ...
    {
        var emps = Database.GetEmps();
        var depts = Database.GetDepts().Where(d => d.Loc == "CHICAGO").ToList().Select(d => d.DeptNo);

        List<Emp> result = emps.Where(e => depts.Contains(e.DeptNo)).ToList(); 
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps
            .Join(grades,e=> true,g=> true,(e, g) => new { e.EName, e.Sal, g.Grade, g.Losal, g.Hisal })
            .Where(r => r.Sal >= r.Losal && r.Sal <= r.Hisal).ToList();
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = emps.GroupBy(e => e.DeptNo).Select(gr => new { DeptNo = gr.Key, AvgSal = gr.Average(e => e.Sal) }).ToList(); 
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = emps
            .Where(e => e.Sal > emps.Where(emp => emp.DeptNo == e.DeptNo).Average(emp => emp.Sal)).Select(e => e.EName).ToList();
